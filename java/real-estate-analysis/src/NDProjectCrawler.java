 ...
 */
public class NDProjectCrawler {

    public static NDMongo mongo = new NDMongo(NDMongo.URI);
    ...
    public static void main(String[] args) {
        NDProjectCrawler.mongo.openCollection("mua_ban_nha_dat");
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    for (String parentUrl : group.urls) {
                        ...
                        try {
                            document = NDProjectCrawler.getDocumentFromUrl(parentUrl);
...
class NDNews {
    ...
    public void parse(String url) throws IOException {
        org.jsoup.nodes.Document jsoupDocument = NDProjectCrawler.getDocumentFromUrl(url);
        ...
        log();
        if (document.size() > 0) {
            insert(NDProjectCrawler.mongo.mongoCollection);
        }
    }
    ...
    public void insert(MongoCollection<Document> mongoCollection) {
        ...
        Document resultDocument = documentFindIterable.first();
        if (resultDocument != null) {
            mongoCollection.updateOne(resultDocument, new Document("$set", document));
